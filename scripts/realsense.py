...
while True:
    ...
    if results.multi_hand_landmarks:
        for i, handLms in enumerate(results.multi_hand_landmarks):
            ...
            def to_pixel(lm):
                return np.array([
                    int(lm.x * len(depth_image_flipped[0])),
                    int(lm.y * len(depth_image_flipped))
                ])

            middle_knuckle = handLms.landmark[9]
            hand_pos = to_pixel(middle_knuckle)
            frame_center = np.array([len(depth_image_flipped[0]) // 2, len(depth_image_flipped) // 2])
            movement = hand_pos - frame_center
            print(f"{hand_side} hand movement from center (x, y): {movement[0]}, {movement[1]}")

            thumb_vec = to_pixel(thumb_tip)
            finger_tips = [index_tip, middle_tip, ring_tip, pinky_tip]
            finger_names = ["Index", "Middle", "Ring", "Pinky"]

            for tip, name in zip(finger_tips, finger_names):
                finger_vec = to_pixel(tip)
                v1 = thumb_vec
                v2 = finger_vec
                dot = np.dot(v1, v2)
                ...
                if norm1 > 0 and norm2 > 0:
                    ...
                    print(f"{hand_side} hand: Angle between Thumb and {name}: {angle_deg:.2f} degrees")
                else:
                    print(f"{hand_side} hand: Could not compute angle for {name}")
    ...
    if results.multi_hand_landmarks:
        ...
        images = cv2.putText(
            images, f"Hands: {number_of_hands}", org, font, fontScale, color, thickness, cv2.LINE_AA
        )
    else:
        images = cv2.putText(
